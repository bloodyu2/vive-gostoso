...
function AdminProfessionalsInner() {
  ...
  async function handleDelete(id: string, name: string) {
    if (!confirm(`Deletar perfil de "${name}"? Esta ação não pode ser desfeita.`)) return
    ...
    <div className="min-h-screen bg-[#FAFAF9]">
      <div className="max-w-5xl mx-auto px-5 md:px-8 py-8">
        ...
        {professionals.length === 0 ? (
          <p className="text-sm text-[#737373]">Nenhum profissional cadastrado ainda.</p>
        ) : (
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-2 justify-end">
                        <button
                          ...
                          title={pro.is_published ? 'Despublicar' : 'Publicar'}
                        >
                          {pro.is_published
