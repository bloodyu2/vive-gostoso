...
export function vcardVive({ papel, nota }: { papel: string; nota: string }): string {
  ...
  const linhas = [
    ...
    `TITLE:${escapar(papel)}`,
    'X-ABShowAs:COMPANY',
    `TEL;TYPE=CELL,VOICE,PREF:+${escapar(VIVE.whatsapp)}`,
    `EMAIL;TYPE=INTERNET,PREF:${escapar(VIVE.email)}`,
    `URL:${escapar(VIVE.site)}`,
    `ADR;TYPE=WORK:${endereco}`,
    `X-SOCIALPROFILE;TYPE=instagram:${escapar(VIVE.instagram)}`,
    `NOTE:${escapar(nota)}`,
    `REV:${REV}`,
    'END:VCARD',
  ]
